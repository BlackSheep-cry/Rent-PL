...
save_traffic_usage() {
    ...
    while IFS=$' \t' read -r port_range limit reset_day _extra || [[ -n "$port_range" ]]; do
        ...
        if [[ "$limit" == "unlimited" ]]; then
            log "INFO" "端口 $port_range: 入站 $in_bytes 字节, 出站 $out_bytes 字节, 总计 $total_bytes 字节, 无限制 Unlimited"
            continue
        fi
...
show_stats() {
    ...
    while IFS=$' \t' read -r port_range limit reset_day _extra; do
        ...
        if [[ "$limit" == "unlimited" || "$cp_limit" == "unlimited" ]]; then
            progress=999
        elif [[ "$cp_limit" =~ ^[0-9.]+$ ]] && (( $(echo "$cp_limit > 0" | bc -l) )); then
        ...
        if [[ "$limit" == "unlimited" || "$cp_limit" == "unlimited" ]]; then
            echo -e "  剩余流量：Unlimited"
            echo -e "  月度限制：Unlimited"
        else
